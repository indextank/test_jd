...
!(async () => {
  $.tuanList = []
  await requireConfig();
  if (helpAuthor) await getAuthorShareCode('https://raw.githubusercontent.com/indextank/test_jd/master/jd_zz.json');
  //if (helpAuthor) await getAuthorShareCode('https://raw.githubusercontent.com/indextank/test_jd/master/backUp/zztuan.json');
  if (!cookiesArr[0]) {
...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({ url: `https://raw.githubusercontent.com/indextank/test_jd/master/backUp/zztuan.json`, 'timeout': 10000 }, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
